...
export class LoginService implements ServiceInterface {
  ...
  async get(id: Id, params?: LoginParams) {
    try {
      ...
      if (params!.query?.associate != null) addToLogin++
      ...
      //whether the current identity-provider is a guest. We're using logins + addToLogin being 0 as a proxy for
      //a brand-new login with that email, which should trigger the auto-association, and any total greater than
      //0 will be seen as an email that is established and shouldn't have the auto-association trigger
      if (identityProvider.type === 'email' && logins.total + addToLogin === 0) {
